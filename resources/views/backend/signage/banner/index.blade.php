@extends('layouts.app')
@section('title_page', ucfirst(__('backend.banner')))
@section('description_page', ucfirst( __('backend.banner') . ' list'))
...
<script>
    ...
    var banner_table = $('#banner_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('signage.banner.data') }}",
        columns: [
            { data: 'id', name: 'id' },
        ...
        "lengthChange": false,
        "language": {
            "info": "{{ __('backend.show') }} _START_ {{ __('backend.to') }} _END_ {{ __('backend.from') }} _TOTAL_ {{ __('backend.banners') }}",
            "paginate": {
    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">{{ ucfirst(__('backend.banner') . ' data list') }}</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
                <a href="{{ route('signage.banner.create') }}" type="button" class="btn btn-success">@lang('backend.add') @lang('backend.banner')</a>
                <table id="banner_table" class="table table-bordered table-hover">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>@lang('backend.banner_page.title')</th>
                            <th>@lang('backend.banner_page.title_original')</th>
